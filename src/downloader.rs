...
pub async fn download_segments(
    ...
    for (i, segment) in segments.iter().enumerate() {
        ...
        let segment_url = match base_url.join(&segment_uri) {
            Ok(url) => url,
            Err(e) => {
                return vec![Err(anyhow!("无法解析分段URL: {} - 错误: {}", segment_uri, e))];
            }
        };
                    ...
                    let key_url = match Url::parse(&ki.uri) {
                        ...
                        Err(_) => {
                            ...
                            match base_url.join(&ki.uri) {
                                Ok(url) => url,
                                Err(e) => return Err(anyhow!("无法解析密钥URL: {} - 错误: {}", ki.uri, e))
                            }
                        }
                    };
...
/// 下载单个分段
async fn download_segment(client: Arc<Client>, url: &Url, path: &Path, key: Option<&[u8]>, iv: Option<&[u8]>) -> Result<()> {
